...
export default {
  ...
  methods: {
    login: debounce(async function () {
      ...
      if (!isOk) return (this.message = "Email incorrecto.");
      ...
      if (!response.ok) {
        if (response.status == 400) data = await response.json();
        else if (response.status == 401) this.logout();
        else
          this.message =
            "Se ha presentado un inconveniente por favor vuelva intentar en un minuto";
      } else {
